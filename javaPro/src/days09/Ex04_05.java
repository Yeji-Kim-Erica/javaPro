...
public class Ex04_05 {
	...
	public static void main(String[] args) {
		...
		if (ㅍ == checkSum) System.out.println("검증 주민번호 O");
		else System.out.println("검증 실패된 주민번호 X");
	...
	private static int getAmericanAge(String rrn) {
		...
		/*
		 * int age = getCountingAge(rrn) - 1; int bMonth =
		 * Integer.parseInt(rrn.substring(2, 4)); int bDay =
		 * Integer.parseInt(rrn.substring(4, 6));
		 * 
		 * LocalDate d = LocalDate.now(); int currentMonth = d.getMonthValue(); int
		 * currentDay = d.getDayOfMonth();
		 * 
		 * if (bMonth < currentMonth) return age; else if (bMonth == currentMonth)
		 * return (bDay <= currentDay) ? age : age - 1; else return age - 1;
		 */
	}
