...
public class Ex01_01 {
	...
	public static void main(String[] args) throws IOException {
		...
		String[] list = sticks.split(" ");
		
		System.out.println(n);
		
		int[] stickList = new int[n];
		for (int i = 0; i < list.length; i++) {
			stickList[i] = Integer.parseInt(list[i]);
		} // for i
		
		while (stickList.length > 1) {
			stickList = cutOperation(stickList);
		} // while
		
	} // main

	private static int[] cutOperation(int[] stickList) {
		int[] sortedList = stickList.clone();
		Arrays.sort(sortedList);
		int min = sortedList[0];
		
		ArrayList<Integer> list = new ArrayList<Integer>();
		for (int i = 0; i < stickList.length; i++) {
			list.add(stickList[i]-min);
		} // for i
		list.removeIf((t) -> t.equals(0));
...
각각 0 이상의 정수를 길이로 하는 N개의 막대기가 있다.
